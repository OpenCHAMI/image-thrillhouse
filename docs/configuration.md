...
**`.git` is always excluded**, from both the copy and the tag, as if `"**/.git"` were the first entry in `excludes`. Git bookkeeping is per-checkout state rather than source: the index and reflogs are rewritten by every clone, fetch, and checkout, so hashing them would hand you a new tag and a rebuild with no source change behind it. Everything alongside it — `.gitignore`, `.gitkeep`, `.gitmodules`, `.gitattributes` — is content and is copied and hashed normally.

The default matters most when `src` is **vendored as a submodule**. `.git` takes a different shape depending on how the tree was obtained — a pointer file naming the submodule's path inside the superproject, a real directory for a standalone clone, nothing at all for a release tarball — so hashing it splits one tree into three tags, and a developer's checkout, CI's recursive clone, and a downstream consumer each rebuild the same bytes under a different name. Excluding it collapses those to one tag. Relocating the submodule within its superproject rewrites the pointer, and that no longer moves the tag either.

Two side benefits: the image doesn't carry a `.git` pointer dangling at a path that doesn't exist inside the container, and a CI clone's `.git/config` — which on GitHub Actions holds a credential in `http.<url>.extraheader` — can't end up in a published image.

There is no opt-out, on the same terms as ansible's roles and inventory trees (see [Ansible content and the tag](#ansible-content-and-the-tag)). `excludes` cannot serve as one — it only subtracts — and a `"!.git"` negation is worse than no escape hatch: buildah would re-include the tree while the tag hasher still skips it, leaving the tag covering less than the image, the one direction that ships stale content. If you have a build that genuinely needs git metadata inside the image, open an issue; that is a config field worth adding on a real use case rather than on speculation.
...
- **Version-control metadata is excluded.** `.git` is skipped anywhere in a hashed roles or inventory tree, the same default [`layer.directories`](#layerdirectories) applies, and for the same reason: it is per-checkout state, not content. Roles vendored as **submodules** are hashed normally — the working tree is what counts, and the submodule's `.git` pointer file is the only thing dropped. Unlike `layer.directories` there is no opt-out here, and there are no user-facing `excludes` either: ansible demonstrably ignores `.git`, but nothing says it ignores an arbitrary pattern, and an exclude that drops a role the playbook does run would under-hash the layer.
