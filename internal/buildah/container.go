...
// NewContainer creates and mounts a container from the given base image
// ("scratch" or any pullable reference). The caller must Delete() it.
//
// The working container's name is generated by buildah; the config's image name
// only appears in the committed tags.
...
// RunScript stages a script in the container's /tmp, runs it, and removes it.
//
// RunOptions (e.g. WithEnv) are forwarded only to the execution step — chmod and
// rm don't need the caller's env.
...
// WriteFile writes one config.File into the container, resolving its content
// from Content, Src, or URL (checked in that order) and staging it through a
// host temp file for buildah.Add.
...
func (c *Container) CommitWithLabelsTags(ctx context.Context, name string, tags []string, labels map[string]string) (string, error) {
	...
	// Add labels if provided (idempotent when the builder already applied
	// them via SetLabels before the publish loop).
	c.SetLabels(labels)
...
// SetLabels applies OCI image labels to the underlying builder's image
// config. Labels persist across every subsequent Commit / CommitToRegistry,
// so applying them once before publishing covers all destinations.
...
// Delete unmounts the container, deletes it, and releases the store handle.
//
// Every step is best-effort and runs even if an earlier one failed, so a stuck
// unmount doesn't compound into a larger leak. Failures are logged at WARN
// rather than swallowed — silent failure here hid storage leaks that only
// surfaced as disk pressure days later.
...
// GetIsolation honours BUILDAH_ISOLATION ("chroot", "rootless", "oci"), falling
// back to buildah's own choice.
...
// CommitToRegistry pushes straight to a remote registry, bypassing local
// storage — cheaper in CI, where the local copy is never used again.
